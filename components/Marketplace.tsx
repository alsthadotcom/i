...
import { OpportunityCard } from './ui/card-12';
            ...
            {!loading && items.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {items.map((item, index) => (
                        <OpportunityCard
                            key={item.idea_id}
                            status={item.mvp ? 'MVP Ready' : 'Available'}
                            postedBy={{
                                name: item.username ? item.username.replace('@', '') : 'Unknown User',
                                avatarUrl: `https://api.dicebear.com/7.x/initials/svg?seed=${item.username || 'User'}`,
                                company: 'Indie Creator', // Placeholder
                            ...
                            deadline={new Date(item.created_at).toLocaleDateString()}
                            matchPercentage={Math.round(item.overall_score || 0)}
                            rating={parseFloat(((item.overall_score || 0) / 20).toFixed(1))}
                            tags={[item.category || 'Business', ...(item.mvp ? ['MVP Included'] : [])]}
                            description={item.description}
                            recruiter={{
                                name: 'Ida AI',
                                avatarUrl: 'https://api.dicebear.com/7.x/bottts/svg?seed=Ida',
                                company: 'AI Broker',
                                location: 'Virtual',
                            }}
                            onAccept={() => handleItemClick(item)}
                            onDecline={() => { }} // Optional: Implement save/dismiss
